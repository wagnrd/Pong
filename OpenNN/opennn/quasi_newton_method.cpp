...
/// @param new_inverse_Hessian_approximation_method_name Name of inverse Hessian approximation method.
    ...
    void QuasiNewtonMethod::set_inverse_Hessian_approximation_method(
            const string& new_inverse_Hessian_approximation_method_name )
    {
...
/// @param new_return_minimum_selection_error_neural_network True if the final model will be the neural network with the minimum selection error, false otherwise.
    ...
    void QuasiNewtonMethod::set_return_minimum_selection_error_neural_network(
            const bool& new_return_minimum_selection_error_neural_network )
    {
...
/// @param old_inverse_Hessian Inverse Hessian at the other point of the error function.
    ...
    Matrix<double> QuasiNewtonMethod::calculate_inverse_Hessian_approximation( const Vector<double>& old_parameters,
                                                                               ...
                                                                               const Matrix<double>& old_inverse_Hessian ) const
    {
        switch ( inverse_Hessian_approximation_method )
        {
            case DFP:
            {
                return ( calculate_DFP_inverse_Hessian( old_parameters, parameters, old_gradient, gradient,
                                                        old_inverse_Hessian ) );
            }

            case BFGS:
            {
                return ( calculate_BFGS_inverse_Hessian( old_parameters, parameters, old_gradient, gradient,
                                                         old_inverse_Hessian ) );
            }
        }
   ...
   else if(columns_number != parameters_number)
   {
      ...
      throw logic_error(buffer.str());
        ...
        inverse_Hessian_approximation +=
                parameters_difference.direct( parameters_difference ) / parameters_dot_gradient;
...
//   }
        ...
        inverse_Hessian_approximation -=
                ( Hessian_dot_gradient ).direct( Hessian_dot_gradient ) / gradient_dot_Hessian_dot_gradient;
...
/// Returns the pointer to the quasi-Newton method object required by the corresponding results structure.
    ...
    void QuasiNewtonMethod::QuasiNewtonMethodResults::set_quasi_Newton_method_pointer(
...
/// Training occurs according to the training operators, training parameters and stopping criteria.
    ...
    QuasiNewtonMethod::QuasiNewtonMethodResults* QuasiNewtonMethod::perform_training()
    {
        ...
        const size_t selection_instances_number = loss_index_pointer->get_data_set_pointer()
                                                                    ->get_instances()
                                                                    .get_selection_instances_number();
           ...
           else
           {
               inverse_Hessian = calculate_inverse_Hessian_approximation(old_parameters, parameters, old_gradient, gradient, old_inverse_Hessian);
                ...
                switch ( inverse_Hessian_approximation_method )
                {
                    case DFP:
                    {
                        inverse_Hessian.update_DFP_inverse_Hessian( old_parameters, parameters, old_gradient,
                                                                    gradient );
                    }
                        break;
                    ...
                    case BFGS:
                    {
                        inverse_Hessian.update_BFGS_inverse_Hessian( old_parameters, parameters, old_gradient,
                                                                     gradient );
                    }
                        break;
                }
            ...
            directional_point = learning_rate_algorithm.calculate_directional_point( training_loss, training_direction,
...
//       training_loss = directional_point[1];
            ...
            if ( stop_training )
            {
                ...
                if ( display )
                {
                    cout << "Parameters norm: " << parameters_norm << "\n" << "Training loss: " << training_loss << "\n"
                         ...
                         << write_elapsed_time( elapsed_time ) << endl;
            ...
            else if ( display && epoch % display_period == 0 )
            {
                cout << "Epoch " << epoch << ";\n" << "Parameters norm: " << parameters_norm << "\n"
                     << "Training loss: " << training_loss << "\n" << "Gradient norm: " << gradient_norm << "\n"
                     << loss_index_pointer->write_information() << "Training rate: " << training_rate << "\n"
...
/// the stopping criteria and other user stuff concerning the quasi-Newton method object.
    ...
    tinyxml2::XMLDocument* QuasiNewtonMethod::to_XML() const
    {
        ...
        // Training rate algorithm
        {
            ...
            const tinyxml2::XMLElement* learning_rate_algorithm_element = learning_rate_algorithm_document->FirstChildElement(
...
// void from_XML(const tinyxml2::XMLDocument&) const method
    ...
    void QuasiNewtonMethod::from_XML( const tinyxml2::XMLDocument& document )
    {
        ...
        const tinyxml2::XMLElement* return_minimum_selection_error_neural_network_element = root_element->FirstChildElement(
                "ReturnMinimumSelectionErrorNN" );

        if ( return_minimum_selection_error_neural_network_element )
        {
            string new_return_minimum_selection_error_neural_network = return_minimum_selection_error_neural_network_element
                    ->GetText();

            try
            {
                set_return_minimum_selection_error_neural_network(
                        new_return_minimum_selection_error_neural_network != "0" );
            }
            catch ( const logic_error& e )
            {
                cerr << e.what() << endl;
        ...
        const tinyxml2::XMLElement* apply_early_stopping_element = root_element->FirstChildElement(
                "ApplyEarlyStopping" );
